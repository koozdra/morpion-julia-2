...
function q_sample_episode(board_template, q_table)
    gym = new_gym(board_template)
    ...
    while !isempty(gym.possible_moves)
        ...
        action = if is_explore
            ...
            q_select_possible_move(q_table, gym)
        end
...
function q_generate_key(state_action::Array{Move})
    lines_hash(state_action)
end

function q_select_possible_move(q_table, gym)
    gym.possible_moves[argmax(map(function (possible_move)
        # (hash_key, move_position, visits, moves) = state
        # score = length(moves)
        # # [score - (visits / score_visits_decay), -visits, rand]
        # [score - (visits / score_visits_decay)]
        state_action = [gym.taken_moves;[possible_move]]
        q_key = q_generate_key(state_action)
        q_value = if haskey(q_table, q_key)
            q_table[q_key]
        else
            0
        end
        q_value
    end, gym.possible_moves))]
end

# next_q_values = map(possible_move -> get_q_value(q_table, get_q_key(state, possible_move)), possible_moves)
#     max_next_q_value = maximum(next_q_values)

function q_get_table_value(q_table, q_key)
    if haskey(q_table, q_key)
        q_table[q_key]
    else
        0
...
function q_max_next_q_value(q_table, gym)
    next_q_values = map(function (possible_move)
        next_state_action = [gym.taken_moves;[possible_move]]
        q_get_table_value(q_table, q_generate_key(next_state_action))
    end, gym.possible_moves)
    
    maximum(next_q_values)
    
end

function q_table_greedy_eval(board_template, q_table)
    gym = new_gym(board_template)

    while !isempty(gym.possible_moves)
        # action = select_random_possible_move(gym)

        action = q_select_possible_move(q_table, gym)
